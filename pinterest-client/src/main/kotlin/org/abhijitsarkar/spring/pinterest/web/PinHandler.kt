...
class PinHandler(val pinterestService: PinterestService, val cache: Cache) {
    ...
    fun create(request: ServerRequest): Mono<ServerResponse> {
        ...
        return if (isEmpty(sessionId)) {
            ...
            val accessToken = cache.get(sessionId, String::class.java)
            pinterestService.findOrCreateBoard(boardName, accessToken)
                                ...
                                .flatMap {
                                    ...
                                    filePart!!.content()
                                            ...
                                            .flatMap { bodyBytes ->
                                                ...
                                                val extension = filename?.takeLastWhile { it != '.' }
                                                        ?.let { ".$it" }

                                                logger.debug("Found filename: {}, deduced extension: {}.",
                                                        filename, extension)
